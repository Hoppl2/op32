...
Begin VB.Form frmSchwellProtInfo 
   Caption         =   "Form1"
   ClientHeight    =   4335
   ClientLeft      =   60
   ClientTop       =   345
   ...
   Begin VB.CommandButton cmdEsc 
      ...
      Width           =   1200
   End
   Begin MSFlexGridLib.MSFlexGrid flxSchwellProtInfo 
      Height          =   2280
      Left            =   0
      TabIndex        =   0
      Top             =   0
   ...
   End
End
Attribute VB_Name = "frmSchwellProtInfo"
...
Private Const DefErrModul = "SCHWELLPROTINFO.FRM"
...
With flxSchwellProtInfo
    sInfoTyp$ = Left$(SchwellInfoSuch$, 1)
    If (sInfoTyp$ = "S") Or (sInfoTyp$ = "U") Then
        .Cols = 3
        .Rows = 2
        .FixedRows = 1
        .FixedCols = 1
        ...
        Do While Not EOF(DRUCKHANDLE%)
            ...
            If (Left$(h$, 5) = SchwellInfoSuch$) Then
                h$ = Mid$(h$, 6)
                .TextMatrix(row%, 1 + iToggle%) = Trim(h$)
                If (iToggle% = 0) Then
                    iToggle% = 1
                Else
                    row% = row% + 1
   ...
   Else
        ...
        Do While Not EOF(DRUCKHANDLE%)
            ...
            If (Left$(h$, 1) = "*") Then
                Exit Do
            ElseIf (Left$(h$, 5) = SchwellInfoSuch$) Then
                .Rows = .Rows + 1
                .row = .Rows - 1
                If (Mid$(h$, 7, 1) = "1") Then
...
Me.Width = flxSchwellProtInfo.Left + flxSchwellProtInfo.Width + 2 * wpara.LinksX
Caption = SchwellInfoName$

With cmdEsc
    .Top = flxSchwellProtInfo.Top + flxSchwellProtInfo.Height + 150 * wpara.BildFaktor
    .Width = wpara.ButtonX%
