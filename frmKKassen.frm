...
For i% = 0 To UBound(AvKassen$)
  If Val(Left$(AvKassen$(i%), 2)) = KasseRec!Typ Then
    cmbKTyp.ListIndex = i% + 1
    Exit For
  End If
Next i%
...
With flxAbrech
    ...
    If Val(KKSatz) > 0 Then
      ...
      Do While Not ds.EOF
        ...
        Quote# = ImportQuote#(GesFAM#, ImpFähig#, Left(sCheckNull(ds!Monat), 4))
...
'    Kkasse.Name = txtKName.text
    ...
    KasseRec!Typ = 1
'    Kkasse.Typ = 1
    If Val(Right(cmbKTyp.List(cmbKTyp.ListIndex), 2)) > 0 Then
      KasseRec!Typ = Val(Right(cmbKTyp.List(cmbKTyp.ListIndex), 2))
'      Kkasse.Typ = Val(Right(cmbKTyp.List(cmbKTyp.ListIndex), 2))
    End If
...
cmbKTyp.AddItem ""
For i = 0 To UBound(AvKassen$)
  h$ = Mid$(AvKassen$(i), 3) + Space(100) + Left$(AvKassen$(i), 2)
  cmbKTyp.AddItem h$
Next i
