...
Begin VB.Form frmLennartzAuswahl 
   AutoRedraw      =   -1  'True
   Caption         =   "Exportierte Lennartz-Rezepturen"
   ClientHeight    =   5775
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6600
   ...
   Begin VB.PictureBox picControlBox 
      ...
      Index           =   2
      Left            =   4080
      Picture         =   "LennartzAuswahl.frx":0000
      ScaleHeight     =   240
      ScaleWidth      =   240
      TabIndex        =   11
      TabStop         =   0   'False
   ...
   Begin VB.PictureBox picControlBox 
      ...
      Index           =   1
      Left            =   3840
      Picture         =   "LennartzAuswahl.frx":00B9
      ScaleHeight     =   240
      ScaleWidth      =   240
      TabIndex        =   10
      TabStop         =   0   'False
   ...
   Begin VB.PictureBox picControlBox 
      ...
      Index           =   0
      Left            =   3600
      Picture         =   "LennartzAuswahl.frx":016D
      ScaleHeight     =   240
      ScaleWidth      =   240
      TabIndex        =   9
      TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmLennartzAuswahl"
...
Private Const DefErrModul = "LENNARTZAUSWAHL.FRM"
...
SumPreis = xVal(flxTaxMuster(0).TextMatrix(row%, 3))
SumPreisZuz = xVal(flxTaxMuster(0).TextMatrix(row%, 4))

LennartzPzn = xVal(flxTaxMuster(0).TextMatrix(row%, 6))

If (LennartzPzn = 9999011) Then
    LennartzTxt = "Rezeptur"
ElseIf (LennartzPzn = 6460702) Then
    LennartzTxt = "Abfüllung"
ElseIf (LennartzPzn = 6460665) Then
    LennartzTxt = "Cannabisblüten in Zubereitungen"
ElseIf (LennartzPzn = 6460694) Then
    LennartzTxt = "Cannabisblüten unverändert"
End If

LennartzDatei = flxTaxMuster(0).TextMatrix(flxTaxMuster(0).row, 0)
...
With flxTaxMuster(0)
    .Rows = 2
    .FixedRows = 1
    .FormatString = "|<Rezeptur-Name|^Menge|>Preis|>PreisOhneBtm|^Datum|^SonderPzn|<Kunde||"
    .Rows = 1
    .ColWidth(0) = 0
    .ColWidth(1) = TextWidth(String(50, "A"))
    .ColWidth(2) = 0
    .ColWidth(3) = TextWidth(String(10, "9"))
    .ColWidth(4) = 0
    .ColWidth(5) = TextWidth(String(20, "A"))
    .ColWidth(6) = TextWidth(String(15, "A"))
    .ColWidth(7) = TextWidth(String(25, "A"))
    .ColWidth(8) = 0
...
Dim Erstellungsdatum$, Bezeichnung$, SonderPzn$, Kunde$, PreisBrutto$, PreisBruttoOhneBtm$, sXML$
...
Dim DirMask$, EntryName$
Dim SearchHandle&
Dim FindDataRec As WIN32_FIND_DATA
Dim erg%

DirMask$ = LennartzPfad + "\*.xml"
...
Do
    ...
    sXML = ReadFileToText(LennartzPfad + "\" + EntryName)
    
    h2 = XmlAbschnitt(sXML, "ERSTELLUNGSDATUM")
    If (h2 <> "") Then
        If (Left(h2, 1) = "#") Then
            h2 = Mid(h2, 2)
        End If
        ...
        End If
        Dim iDat As Date
        iDat = CDate(Mid(h2, 4, 2) + "." + Mid(h2, 1, 2) + "." + Mid(h2, 7))
        h2 = Format(iDat, "dd.MM.yyyy HH:mm")
    End If
    
    Bezeichnung = XmlAbschnitt(sXML, "BEZEICHNUNG")
    SonderPzn = XmlAbschnitt(sXML, "SONDERPZN")
    Kunde = XmlAbschnitt(sXML, "KUNDE")
    
    PreisBrutto = XmlAbschnitt(sXML, "PREISBRUTTO")
    PreisBruttoOhneBtm = XmlAbschnitt(sXML, "PREISBRUTTOOHNEBTM")
   
    h = EntryName + vbTab + Bezeichnung + vbTab + vbTab + PreisBrutto + vbTab + PreisBruttoOhneBtm + vbTab + h2 + vbTab + SonderPzn + vbTab + Kunde + vbTab
