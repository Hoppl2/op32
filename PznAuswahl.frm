...
Begin VB.Form frmPznAuswahl 
   AutoRedraw      =   -1  'True
   Caption         =   "Auswahl spezieller PZNs"
   ClientHeight    =   4245
   ClientLeft      =   1650
   ClientTop       =   1545
   ClientWidth     =   6570
   ...
   Begin VB.CommandButton cmdEsc 
      ...
      Width           =   1200
   End
   Begin MSFlexGridLib.MSFlexGrid flxPznAuswahl 
      Height          =   2700
      Index           =   0
      Left            =   0
      TabIndex        =   0
   ...
   End
End
Attribute VB_Name = "frmPznAuswahl"
...
Private Const DefErrModul = "PZNAUSWAHL.FRM"
...
With flxPznAuswahl(1)
    .CellFontBold = True
    PznAuswahlName$ = .TextMatrix(.row, 0)
    PznAuswahlWert$ = .TextMatrix(.row, 1)
End With
...
Private Sub flxPznAuswahl_DblClick(index As Integer)
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Call DefErrFnc("flxPznAuswahl_DblClick")
Call DefErrMod(DefErrModul)
On Error GoTo DefErr
GoTo DefErrEnd
DefErr:
...
Private Sub flxPznAuswahl_RowColChange(index As Integer)
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Call DefErrFnc("flxPznAuswahl_RowColChange")
Call DefErrMod(DefErrModul)
On Error GoTo DefErr
GoTo DefErrEnd
DefErr:
...
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
If (index = 0) Then
    If (flxPznAuswahl(0).Redraw) Then
        Call InsertSonderDateien
    End If
End If
...
With flxPznAuswahl(0)
    .Rows = 2
    .FixedRows = 1
    
    .Top = wpara.TitelY
    .Left = wpara.LinksX
    .Height = .RowHeight(0) * 12 + 90
    ...
    .ColWidth(1) = 0
    .Rows = 1
    .AddItem "Hilfsmittel-Nr individuell"
    .AddItem "Hilfsmittel-Nr Gruppen"
    .AddItem "Sonder-PZN"
End With
...
With flxPznAuswahl(1)
    ...
    .Top = wpara.TitelY
    .Left = flxPznAuswahl(0).Left + flxPznAuswahl(0).Width + 300
...
cmdOk.Top = flxPznAuswahl(0).Top + flxPznAuswahl(0).Height + 150
cmdEsc.Top = cmdOk.Top

Me.Width = flxPznAuswahl(1).Left + flxPznAuswahl(1).Width + 2 * wpara.LinksX
...
'        .Left = Me.ScaleWidth - .Width - 150
        .Top = flxPznAuswahl(0).Top + flxPznAuswahl(0).Height + iAdd + 600
...
'        .Left = Me.ScaleWidth - .Width - 150
    ...
    With flxPznAuswahl(0)
        RoundRect hdc, (.Left - iAdd) / Screen.TwipsPerPixelX, (.Top - iAdd) / Screen.TwipsPerPixelY, (flxPznAuswahl(1).Left + flxPznAuswahl(1).Width + iAdd) / Screen.TwipsPerPixelX, (.Top + .Height + iAdd) / Screen.TwipsPerPixelY, 20, 20
    End With
...
Sub InsertSonderDateien()
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Call DefErrFnc("InsertSonderDateien")
Call DefErrMod(DefErrModul)
On Error GoTo DefErr
GoTo DefErrEnd
DefErr:
...
With flxPznAuswahl(1)
    ...
    .Rows = 2
    .FixedRows = 1
    .FormatString = "<" + flxPznAuswahl(0).text + "|<"
    .Rows = 1
    .Cols = 2
    .ColWidth(0) = .Width
    .ColWidth(1) = 0
    ...
    row% = flxPznAuswahl(0).row
    Call ActProgram.FlxPznAuswahlBefuellen(row% - 1)
...
With flxPznAuswahl(1)
    For i% = 1 To (.Rows - 1)
        If (.TextMatrix(i%, 1) = PznAuswahlWert$) Then
            .row = i%
            .TopRow = i%
            .CellFontBold = True
            flxPznAuswahl(0).CellFontBold = True
        End If
    Next i%
End With
