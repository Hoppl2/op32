            ...
            If (rInd%) And (ww.pzn = mnr$(i%)) And (ww.aktivlief = Lief%) Then
                ...
                Else
                    ...
                    If (bm% = 0) And (nm% = 0) Then     '1.60 Merkzettel
                        Call MerkzettelSpeichern(pzn$, Mid$(text$, 9), xcBestDatum%, xcBestDatum%, Lieferant%, 0)
                        ww.status = 0
                    Else
                        ww.status = 2
                        ww.Lief = Lief%
...
'                            ww.WuFertig = 0
                    ...
                    If (MacheEtiketten%) Then
                        pp% = 0
                        If (para.Land = "D") Or (MacheAlleEtiketten%) Then
                            If (ssatz% <> 0) Then If (InStr("AFP", ass.pp) <> 0) Then pp% = True
                            If (InStr(para.PosAktivWG, ww.wg) <> 0) Then pp% = True
                        ElseIf (OrgPzn$ <> "9999999") Then
                            FabsErrf% = ast.IndexSearch(0, OrgPzn$, FabsRecno&)
                            If (FabsErrf% = 0) Then
                                ast.GetRecord (FabsRecno& + 1)
                            Else
                            ...
                            If (FabsErrf% = 0) Then
                                ...
                                If (wg% <> 1) And (wg% <> 2) And (wg% <> 5) Then pp% = True
                                If (InStr(ast.kasz, "P") <> 0) Then pp% = True
                                If (Aufschlag% = 9) Then pp% = True
                                If (ast.aep = 0) Or (ast.AVP = 0) Then pp% = True
                            End If
                        End If
...
If (rInd%) Then
    ...
    Else
        ...
        text$ = pzn$ + " " + Left$(ww.txt, 33) + " " + Mid$(ww.txt, 34, 2)
        If (bm% = 0) And (nm% = 0) Then     '1.60 Merkzettel
            Call MerkzettelSpeichern(pzn$, Mid$(text$, 9), xcBdatum%, xcBdatum%, Lieferant%, 0)
            ww.status = 0
        Else
            ww.status = 2
            ww.Lief = Lief%
...
Sub MerkzettelSpeichern(pzn$, text$, dat%, dt%, Lief%, Lm%)
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Call DefErrFnc("MerkzettelSpeichern")
Call DefErrMod(DefErrModul)
On Error GoTo DefErr
GoTo DefErrEnd
DefErr:
...
If (InStr(para.Benutz, "t") > 0) Then
    ...
    MerkzettelRec!pzn = pzn$
    MerkzettelRec!txt = text$
    MerkzettelRec!BestellDatum = MakeDateStr$(dat%)
    MerkzettelRec!LieferDatum = MakeDateStr$(dt%)
    MerkzettelRec!Lief = Lief%
    MerkzettelRec!Lm = Lm%
...
'    If warten% Then LOCATE 17, 73: Print Left$(Time$, 5);
    ...
    If (ret% = 0) And (Mid$(RecStr$, 1, 4) = "BUSY") Then
        ...
        If (AnzBesetzt% < 3) Then
            deb$ = "Neuerlicher Versuch in 90 Sekunden ...": Call StatusZeile(deb$)
            Call IsdnPause(90)
            status$ = "Telefonnummer " + TelGh$ + " wird gewählt.": Call StatusZeile(status$)
            SendStr$ = AustriaDialP$ + TelGh$ + AustriaDialS$
            ret% = TransmitBlock%(SendStr$, RecStr$)
