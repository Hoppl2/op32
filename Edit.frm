...
Begin VB.Form frmEdit 
   ...
   Begin VB.ListBox lstEdit 
      ...
      Width           =   1215
   End
   Begin MSFlexGridLib.MSFlexGrid flxMultiEdit 
      Height          =   615
      Left            =   3360
      TabIndex        =   4
      Top             =   2160
...
'        EditTxt$ = .TextMatrix(.row, 0) + vbTab + .TextMatrix(.row, 1) + vbTab + .TextMatrix(.row, 2)
    End With
ElseIf (flxMultiEdit.Visible) Then
    EditTxt$ = ""
    EditAnzGefunden% = 0
    EditGef%(0) = 0
    With flxMultiEdit
...
Private Sub flxEdit_KeyPress(KeyAscii As Integer)
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Call DefErrFnc("flxEdit_KeyPress")
Call DefErrMod(DefErrModul)
On Error GoTo DefErr
GoTo DefErrEnd
DefErr:
...
If (InStr("ABCDEFGHIJKLMNOPQRSTUVWXYZ", UCase(Chr$(KeyAscii))) > 0) Then
    ...
    SearchLetter = UCase(Chr$(KeyAscii))
            
    gef% = -1
    With flxEdit
        For i% = (.row + 1) To (.Rows - 1)
            ch$ = UCase(Left$(.TextMatrix(i%, 4), 1))
...
Private Sub flxMultiEdit_KeyPress(KeyAscii As Integer)
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Call DefErrFnc("flxMultiEdit_KeyPress")
Call DefErrMod(DefErrModul)
On Error GoTo DefErr
GoTo DefErrEnd
DefErr:
...
Private Sub flxMultiEdit_DblClick()
'DefErr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Call DefErrFnc("flxMultiEdit_DblClick")
Call DefErrMod(DefErrModul)
On Error GoTo DefErr
GoTo DefErrEnd
DefErr:
...
Call flxMultiEdit_KeyPress(vbKeySpace)
